...
@section('content')
    <table>
        <thead>
            <th colspan="3">
                Ficha da Instrução Individual de Qualificação
            </th>
        </thead>
        <tbody>
            <tr>
    ...
    <div class="div-oii col-12 row">
        ...
        @if ($militar->getInformacoesFiiq()->isNotEmpty())
            @php
                $valor_chunk = 1;
                if (count($militar->getInformacoesFiiq()) > 5) {
                    $valor_chunk = ceil(count($militar->getInformacoesFiiq()) / 5);
                }
                $grupoResultados = $militar->getInformacoesFiiq()->chunk($valor_chunk);
            @endphp
