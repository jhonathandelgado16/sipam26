...
@section('content')
    ...
    <div class="row margin-bottom-10">
        <div class="col-lg-12 margin-tb">
            <div class="row">
                <h2 class="title-caderneta">Preencher Ficha da Instrução Individual Básica</h2>
            </div>
        </div>
    </div>
    ...
    <div class="row justify-content-center">
        ...
        {!! Form::open(['route' => ['fiib.realizar', $militar->id], 'method' => 'POST']) !!}

        <div class="row col-12">
            @php
                $valor_chunk = 1;
                if (count($objetivos_dentro_da_fiib) > 4) {
                    $valor_chunk = ceil(count($objetivos_dentro_da_fiib) / 4);
                }
                $grupoObjetivos = $objetivos_dentro_da_fiib->chunk($valor_chunk);
                $counter = 0;
            @endphp
                ...
                <div class="col-3">
                    ...
                    @foreach ($grupo as $key => $objetivo_instrucao)
                        <div class="row item-oii text-center">
                            ...
                            <div class="col-5">{{ $objetivo_instrucao->getOII() }}</div>
                            <div class="col-7 row">
                                <div class="col-6">
                                    <input type="radio" value="1" class="btn-check"
                                        name="informacoes[{{ $counter }}][1]"
                                        id="option1-{{ $objetivo_instrucao->id }}" autocomplete="off" checked>
